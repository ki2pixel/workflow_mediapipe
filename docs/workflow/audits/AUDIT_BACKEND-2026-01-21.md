...
| **Faible** | **Code** | Nettoyage `app_new.py` | **RÉSOLU (2026-01-21)** : Création d’une fonction `init_app()` idempotente qui configure le logging, initialise les services et lance les threads `RemoteWorkflowPoller`/`CSVMonitorService` une seule fois (verrou `_app_init_lock`, flag `_polling_threads_started`). Le bloc `__main__` se contente d’appeler `init_app()` avant `APP_FLASK.run(...)`. Validation : `python3 -m py_compile app_new.py`. |
