...
- Les workers multiprocessing chargent automatiquement le `.env` et propagent les limites `STEP5_MEDIAPIPE_MAX_FACES`, `STEP5_MEDIAPIPE_JAWOPEN_SCALE`, `STEP5_MEDIAPIPE_MAX_WIDTH` ainsi que la hiérarchie des modèles STEP5 (`workflow_scripts/step5/models/`). Renseigner ces variables dans `.env` est recommandé pour éviter la saturation CPU ou des incohérences d’analyse.
...
-  - Les moteurs supportés sont : mode MediaPipe par défaut (`STEP5_TRACKING_ENGINE` vide) et `insightface`.
-  - `create_face_engine()` n'instancie plus que `insightface` (ou retourne `None` pour MediaPipe).
  ...
  - `TRACKING_DISABLE_GPU=1` reste le comportement par défaut pour garantir la stabilité v4.1 ; seul InsightFace peut activer le GPU si `STEP5_ENABLE_GPU=1` et `STEP5_GPU_ENGINES` contient `insightface`.
  ...
  - `STEP5_INSIGHTFACE_ENV_PYTHON` permet de relocaliser l’environnement InsightFace GPU-only lorsque `VENV_BASE_DIR` pointe hors du repo.
