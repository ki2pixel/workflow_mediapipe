...
- [2026-02-03 01:34:00] **STEP5 — Simplification moteurs tracking (Phase 2)** : finalisation complète de la simplification STEP5 avec suppression réelle des assets/scripts/tests legacy OpenSeeFace/OpenCV/EOS, nettoyage du code (`face_engines.py` ne garde que `InsightFaceEngine` + factory), retrait des env vars legacy (`config/settings.py`, `.env.example`, `.env`), mise à jour de toute la documentation (pipeline/architecture/guides) et scripts GPU (`validate_gpu_prerequisites.sh`). Tests unitaires alignés et validés. Architecture STEP5 désormais simplifiée : MediaPipe CPU par défaut + InsightFace GPU-only.
