...
| STEP5 | Tracking multi-moteurs | `tracking_env_slim/bin/python` | `tracking_env_slim/bin/python workflow_scripts/step5/run_tracking_manager.py --videos_json_path <path> --tracking_engine <engine>` | `logs/step5/manager_*.log`, `logs/step5/worker_*.log` | `TRACKING_DISABLE_GPU`/`STEP5_ENABLE_GPU` cohérents, modèles présents, `tracking_env_slim` allégé |
...
| STEP7 | Pré-traitement After Effects | `env/bin/python` | `env/bin/python workflow_scripts/step7/preprocess_ae_json.py --log_dir logs/step7 --work_dir projets_extraits` | `logs/step7/*.log` | Fichiers `*_tracking.json` disponibles, `STEP7_*` vars définies |
| STEP8 | Finalisation + archivage | `env/bin/python` | `env/bin/python workflow_scripts/step8/finalize_and_copy.py` | `logs/step8/*.log`, `archives/*/metadata.json` | `OUTPUT_DIR`, `ARCHIVES_DIR` et `ResultsArchiver` configurés |
...
- **Cache et archives** : vérifier que `CACHE_ROOT_DIR` et `ARCHIVES_DIR` pointent vers des emplacements montés avec suffisamment d’espace (>=200 Go recommandés pour STEP5/STEP7/STEP8).
