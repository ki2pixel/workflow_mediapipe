...
- This rule defines PR message format in a way that aligns with commit message convention based on Conventional Commits (`commit-message-format.md`).
- While recommending same style for title `Prefix` and summary as commit messages, it requires structured descriptions in PR body such as "Overview," "Changes," and "Test Content."
...
- In this rule file, `language` is used as a logical name representing language used in PR messages.
- `language = "en"`
- Title and body should be written in language specified by `language` as a rule.
...
PR body is recommended to have following structured sections.
...
- Do not guess from issue titles or branch names alone; clearly state change content, impact scope, and test content in body.
- Even for automatic generation by AI or scripts, use diffs, commit history, and related Issue information as input.
- Determine Prefix and summary to align with commit message convention (`commit-message-format.md`) (avoid semantic inconsistency between commits and PR).
