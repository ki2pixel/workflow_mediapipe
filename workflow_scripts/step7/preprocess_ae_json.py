...
def _load_json(path: Path) -> Optional[Dict[str, Any]]:
    # Fallback to in-memory load for very small configs
    try:
...
def _find_audio_json_for_video_json(video_json_path: Path) -> Optional[Path]:
    if not video_json_path.exists(): return None
    base_name = video_json_path.name
    stem = base_name
    if stem.lower().endswith("_ae.json"): stem = stem[: -len("_ae.json")]
    elif stem.lower().endswith("_tracking.json"): stem = stem[: -len("_tracking.json")]
    elif stem.lower().endswith(".json"): stem = stem[: -len(".json")]
    audio_path = video_json_path.parent / f"{stem}{AUDIO_SUFFIX}"
    return audio_path if audio_path.exists() else None

def _extract_complex_metadata_streaming(file_path: Path) -> Dict[str, Any]:
    out = {}
    try:
        with open(file_path, 'rb') as f:
            for prefix, event, value in ijson.parse(f, use_float=True):
                if prefix in ('fps', 'total_frames') and event in ('number', 'integer'):
                    out[prefix] = value
...
def _stream_index_audio(audio_path: Path) -> Tuple[Dict[int, Dict[str, Any]], Optional[int]]:
    ...
    try:
        ...
        if array_key:
            with open(audio_path, 'rb') as f:
                for frame_obj in ijson.items(f, f"{array_key}.item", use_float=True):
                    frame_num = frame_obj.get("frame")
                    try: frame_i = int(frame_num)
                    except Exception: continue
                    audio_info = frame_obj.get("audio_info")
                    if not isinstance(audio_info, dict): continue
...
def _stream_index_tracking(video_json_path: Path) -> Optional[Dict[str, Any]]:
    meta = _extract_complex_metadata_streaming(video_json_path)
    
    if meta.get("schema") == "ae_preprocessed":
        return _load_json(video_json_path)
        
    array_key = None
    has_data_by_frame = False
    try:
        with open(video_json_path, 'rb') as f:
            for prefix, event, value in ijson.parse(f):
                if prefix == 'dataByFrame' and event == 'start_map':
                    has_data_by_frame = True
                    break
                if event == 'start_array' and prefix in ('frames', 'frames_analysis'):
                    array_key = prefix
                    break
        ...
        with open(video_json_path, 'rb') as f:
            for frame_obj in ijson.items(f, f"{array_key}.item", use_float=True):
                ...
                for obj in tracked:
                    ...
                    label = obj.get("label")
                    if not ((source == "face_landmarker") or (source == "object_detector" and label == "person")):
                        continue
                    obj_id = obj.get("id")
                    if not obj_id: continue
                    
                    try: cx_f = float(obj.get("centroid_x", 0))
                    except Exception: continue
                    
                    try: bbox_w_f = float(obj.get("bbox_width") or 0.0)
                    except Exception: bbox_w_f = 0.0
                    try: bbox_h_f = float(obj.get("bbox_height") or 0.0)
                    except Exception: bbox_h_f = 0.0
                    bbox_surface = max(0.0, bbox_w_f) * max(0.0, bbox_h_f)
                    
                    try: conf_f = float(obj.get("confidence") or 0.0)
                    except Exception: conf_f = 0.0
                    conf_f = max(0.0, min(1.0, conf_f))
                    
                    speakers_out = [s for s in obj.get("video_speakers", []) if isinstance(s, str) and s]
                    
                    out_tracked.append({
                        "id": obj_id, "centroid_x": cx_f, "source": source, "label": label,
                        "video_speakers": speakers_out, "bbox_width": bbox_w_f, "bbox_height": bbox_h_f,
                        "bbox_surface": bbox_surface, "confidence": conf_f
                    })
                if out_tracked:
    ...
    out = meta.copy()
    out["dataByFrame"] = {str(k): v for k, v in data_by_frame.items()}
    out["maxFrame"] = max_frame_seen
    return out

def _load_ae_or_tracking_index(video_json_path: Path) -> Optional[Dict[str, Any]]:
    return _stream_index_tracking(video_json_path)
...
def _compute_video_frame_scale(video_max_frame: int, reference_max_frame: int) -> float:
    if video_max_frame <= 0 or reference_max_frame <= 0: return 1.0
    diff_ratio = abs(float(video_max_frame - reference_max_frame)) / float(reference_max_frame)
    if diff_ratio > 0.05: return float(video_max_frame) / float(reference_max_frame)
    return 1.0
...
def _analyze_layer(
    ...
    if min_frame is None or max_frame is None: return None
    objects_in_layer: Dict[str, Dict[str, Any]] = {}
    try: layer_scale = float(layer_info.get("video_scale", 1.0))
    except Exception: layer_scale = 1.0
    if layer_scale <= 0.0: layer_scale = 1.0
    ...
    for frame in range(min_frame, max_frame + 1):
        ...
        if effective_frame_scale != 1.0:
            ...
            if video_max_frame > 0 and mapped > video_max_frame: mapped = video_max_frame
            video_frame = mapped

        frame_objs = data_by_frame.get(video_frame)
        audio_info = audio_by_frame.get(frame)

        if not isinstance(frame_objs, list): continue
        ...
        for obj in frame_objs:
            ...
            source = obj.get("source")
            label = obj.get("label")
            try: cx_f = float(obj.get("centroid_x"))
            except Exception: continue
            try: bbox_surface = float(obj.get("bbox_surface") or 0.0)
            except Exception: bbox_surface = 0.0
            if bbox_surface < 0.0: bbox_surface = 0.0
            ...
            if obj_id not in objects_in_layer:
                objects_in_layer[obj_id] = {
                    "id": obj_id, "source": source, "label": label,
                    "x_values": [], "bbox_surfaces": [], "confidence_values": [],
                    "audio_confirm_count": 0, "total_bbox_surface": 0.0,
                    "total_confidence": 0.0, "avg_bbox_surface": 0.0,
                    "avg_confidence": 0.0, "video_speakers": speakers,
                }
            ...
            is_eligible_for_audio = (source == "face_landmarker") or (source == "object_detector" and label == "person")
    ...
    if not objects_in_layer: return None

    best_audio_face = None
    best_audio_person = None
    best_face = None
    best_person = None
    best_fallback = None
    ...
    for oid, obj in objects_in_layer.items():
        ...
        if source == "face_landmarker":
            if audio_confirm > 0:
                current_best_score = _compute_presence_score(
                    len(best_audio_face.get("x_values") or []), float(best_audio_face.get("avg_confidence") or 0.0), config
            ...
            if audio_confirm > 0:
                current_best_score = _compute_presence_score(
                    len(best_audio_person.get("x_values") or []), float(best_audio_person.get("avg_confidence") or 0.0), config
                ) if best_audio_person else -1.0
                if (audio_confirm > max_audio_person_confirm or
                   (audio_confirm == max_audio_person_confirm and presence_score > current_best_score)):
    ...
    try: spread_threshold_f = float(config.get("SPREAD_THRESHOLD", DEFAULT_ANALYZER_CONFIG["SPREAD_THRESHOLD"]))
    except Exception: spread_threshold_f = float(DEFAULT_ANALYZER_CONFIG["SPREAD_THRESHOLD"])
    ...
    reason = "fallback"
    if best_audio_face and target is best_audio_face: reason = "audio_confirm_face"
    elif best_audio_person and target is best_audio_person: reason = "audio_confirm_person"
    elif best_face and target is best_face: reason = "face_presence"
    elif best_person and target is best_person: reason = "person_presence"

    return {
        "center_x": stats["average"], "spread": stats["spread"],
        "label_color": label_to_apply, "selected_id": target.get("id"), "reason": reason,
    }

def analyze_manifest(manifest: Dict[str, Any]) -> Dict[str, Any]:
    raw_layers = manifest.get("layers")
    if not isinstance(raw_layers, dict): raise ValueError("manifest.layers doit être un objet")
    cfg = dict(DEFAULT_ANALYZER_CONFIG)
    raw_cfg = manifest.get("config")
    if isinstance(raw_cfg, dict): cfg.update(raw_cfg)
    ...
    for layer_id, layer_info_raw in raw_layers.items():
        if not isinstance(layer_id, str): layer_id = str(layer_id)
        if not isinstance(layer_info_raw, dict): continue

        json_path_raw = layer_info_raw.get("json_path") or manifest.get("json_path")
        if not isinstance(json_path_raw, str) or not json_path_raw: continue
        ...
        if cache_key not in loaded_cache:
            ...
            if not idx:
                ...
                if isinstance(idx.get("audioByFrame"), dict):
                    ...
                    audio_by_frame, audio_max_frame = _stream_index_audio(audio_path) if audio_path else ({}, None)
        ...
        res = _analyze_layer(layer_info_raw, data_by_frame, audio_by_frame, cfg, video_frame_scale, video_max_frame)
        if res is not None: results[layer_id] = res

    return results

def build_ae_payload_streaming(tracking_in: Path, audio_path: Optional[Path]) -> Optional[Dict[str, Any]]:
    idx = _stream_index_tracking(tracking_in)
    if not idx: return None
    
    audio_by_frame, audio_max_frame = _stream_index_audio(audio_path) if audio_path else ({}, None)
    
    out: Dict[str, Any] = {
        "schema": "ae_preprocessed", "schema_version": 1,
        "dataByFrame": idx.get("dataByFrame", {}),
        "maxFrame": idx.get("maxFrame", 0),
        "audioByFrame": {str(k): v for k, v in audio_by_frame.items()},
        "audioMaxFrame": audio_max_frame,
    }
    
    for k in ('fps', 'total_frames', 'tracking_analytics', 'expression_summary', 'temporal_alignment', 'speaker_embeddings'):
        if k in idx: out[k] = idx[k]
        
    return out

def _find_docs_dirs(work_dir: Path, keyword: str) -> List[Path]:
    if not work_dir.is_dir(): return []
    docs_dirs: List[Path] = []
    for project_dir in work_dir.iterdir():
        if not project_dir.is_dir(): continue
        if keyword and keyword not in project_dir.name: continue
        docs_dir = project_dir / "docs"
        if docs_dir.is_dir(): docs_dirs.append(docs_dir)
    return docs_dirs
...
def _resolve_tracking_paths(docs_dir: Path, stem: str) -> Tuple[Optional[Path], Optional[Path]]:
    ...
    if reduced.exists(): return reduced, legacy if legacy.exists() else None
    if legacy.exists(): return legacy, None
    return None, None
...
def preprocess_docs_dir(docs_dir: Path) -> int:
    ...
    for idx, video_path in enumerate(videos, start=1):
        ...
        audio_path_real = audio_path if audio_path.exists() else None

        payload = build_ae_payload_streaming(tracking_in, audio_path_real)
        if payload is None:
            continue
        ...
        written += 1
        print(f"INTERNAL_PROGRESS: {idx}/{len(videos)} items ({int(round((idx / float(len(videos))) * 100))}%) - {video_path.name}")
    return written
...
def main() -> None:
    ...
    parser.add_argument("--base_dir", type=str, default=os.environ.get("BASE_PATH_SCRIPTS", ""), help="Chemin base du projet")
    parser.add_argument("--work_dir", type=str, default=None, help="Chemin explicite vers projets_extraits")
    parser.add_argument("--keyword", type=str, default=os.environ.get("FOLDER_KEYWORD", "Camille"), help="Mot-clé")
    parser.add_argument("--log_dir", type=str, default=str(os.path.join(os.path.dirname(__file__), "..", "..", "..", "logs", "step7")))
    args = parser.parse_args()

    if args.manifest_path:
        if not args.output_path: raise SystemExit("--output_path requis lorsque --manifest_path est fourni")
        manifest_path = Path(args.manifest_path)
        out_path = Path(args.output_path)
        try:
            with open(manifest_path, "r", encoding="utf-8") as f:
                manifest = json.load(f)
            if not isinstance(manifest, dict): raise ValueError("Manifest invalide (doit être un objet JSON)")
    ...
    if args.work_dir: work_dir = Path(args.work_dir)
    else:
