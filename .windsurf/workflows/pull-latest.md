...
1. **Vérifier l'état local**
   ...
   - Si des modifications existent, les committer ou exécuter `run_command "git stash push -m 'pre-pull'"`
2. **S'assurer d'être sur la bonne branche**
   - `run_command "git branch --show-current"`
   - `run_command "git checkout main"` si nécessaire (adapter si vous travaillez sur une autre branche)
