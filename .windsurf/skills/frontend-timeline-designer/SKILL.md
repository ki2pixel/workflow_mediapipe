...
description: Create or adjust the Connected Timeline UI (HTML/CSS/JS) with AppState + DOMBatcher patterns. Use when touching timeline structure, steps styling, auto-scroll, or Timeline interactions.
---
...
5. **Tests** : lancer `npm run test:frontend` (couvre `test_timeline_logs_phase2.mjs`).

## Step Details Panel
- ⚠️ **RETIRED** : Supprimé le 2026-02-04 (voir `memory-bank/progress.md`)
- Le fichier `stepDetailsPanel.js` existe mais n'est plus utilisé
- L'interface conserve Timeline, Logs overlay et contrôles sans panneau latéral
