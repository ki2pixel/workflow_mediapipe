...
### 2.1 File Editing & Reading
- **Reading**: Always read relevant files before making changes to understand context.
- **Editing**:
  - Use the **available file editing tool** provided by the environment (whether it is named `apply_model_patch`, `edit_file`, or `write_file`). **Do not** look for tools strictly named `edit` or `multi_edit`.
  - **Atomic Changes**: If the editing tool uses a patch/diff mechanism, **avoid complex overlapping changes** in a single turn. Split complex refactors into sequential steps to prevent "overlapping hunks" or "conflict" errors.
- **Comment & Clean Code Authority**:
  - **Dead Code**: Remove commented-out code blocks **immediately without asking** (unless it looks like a critical backup).
  - **Documentation**: You have full authority to add "WHY" comments or refactor unclear comments.
  - **Decision Rule**: If unsure whether to touch a comment, default to **cleaning it up** (dead code) or **improving it** (docs). Do not loop on this decision; treat it as a standard cleanup task.

### 2.2 Parallel Execution
- **Safe Parallelism**: You may execute read-only operations (`read_file`, `grep_search`, `search_web`) in parallel when there are no dependencies.
- **Sequential Writes**: **Never** execute file modification commands in parallel on the same file to avoid race conditions, lock errors, or patch conflicts.
...
- **Code Block Handling**:
  ...
  - For new proposed code, show only the minimal copy-pastable unit.
