...
```mermaid
flowchart TD
    A([BEGIN]) --> B[Charger le contexte: lire activeContext.md et progress.md]
    B --> C[Exécuter memorybankprotocol.md: suspendre tâche et résumer session]
    C --> D[Mettre à jour la Memory Bank: utiliser edit_file pour documenter décisions]
    D --> E[Clôturer la session: vérifier état neutre et résumer tâches finalisées]
    E --> F([END])
```
