...
1. **Analyse Contextuelle & Détection d'Intention**
   ...
   - **Détection de Skill** : Analyser la nature de la tâche :
     - Si **Debugging** (bug, crash, erreur, performance) : Charger immédiatement `.sixthskills/debugging-strategies/SKILL.md`.
     - Si **Architecture** : Charger d'abord `.sixthskills/workflow-operator/SKILL.md`, puis chercher les docs d'architecture pertinentes.
     - Si **Feature** : Identifier et charger le(s) SKILL(s) applicables dans `.sixthskills/` (ex: `frontend-timeline-designer`, `logs-overlay-conductor`, `pipeline-diagnostics`, `step4-audio-orchestrator`, `step5-gpu-ops`, `after-effects-scripts`, `after-effects-cep-panel`) avant de chercher les specs fonctionnelles liées.

2. **Recherche Active de Documentation**
   - Identifier les règles spécifiques au projet via `code_search` dans `docs/workflow` et `.sixthrules/codingstandards.md`.
