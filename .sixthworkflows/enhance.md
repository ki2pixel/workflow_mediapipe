...
3.  **Active les "Skills" (.sixthskills/)** : Selon les mots-clés détectés, utilise tes outils (`read_text_file`, `read_multiple_files`, `search_files`, `search`, `advanced-search`) pour charger le contenu des skills spécialisés :

    *   **Si DEBUGGING / ERREUR / CRASH :**
        *   Lis `.sixthskills/debugging-strategies/SKILL.md` avec `read_text_file`.
        *   Cherche les logs récents avec `search` ou `advanced-search`.

    *   **Si ARCHITECTURE / NOUVEAU SERVICE :**
        *   Lis `.sixthskills/workflow-operator/SKILL.md` avec `read_text_file`.
        *   Cherche dans `docs/workflow/` ou `docs/architecture/` avec `search_files`.

    *   **Si FEATURES SPÉCIFIQUES (Ciblez le fichier précis) :**
        *   *Frontend / UI / CSS* → Lis `.sixthskills/frontend-timeline-designer/SKILL.md` avec `read_text_file`
        *   *Logs / Overlay* → Lis `.sixthskills/logs-overlay-conductor/SKILL.md` avec `read_text_file`
        *   *Pipeline / Env* → Lis `.sixthskills/pipeline-diagnostics/SKILL.md` avec `read_text_file`
        *   *Audio / Step 4* → Lis `.sixthskills/step4-audio-orchestrator/SKILL.md` avec `read_text_file`
        *   *Tracking / GPU / Step 5* → Lis `.sixthskills/step5-gpu-ops/SKILL.md` avec `read_text_file`
        *   *After Effects* → Lis `.sixthskills/after-effects-scripts/SKILL.md` ou `.sixthskills/after-effects-cep-panel/SKILL.md` avec `read_text_file`
